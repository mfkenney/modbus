...
const (
	HalfDuplex Duplex   = 1
	FullDuplex Duplex   = 2
	ActiveHigh RtsState = 0
	ActiveLow  RtsState = 1
)

type RS485Configuration struct {
	// Communication mode, FullDuplex or HalfDuplex
	Mode Duplex
	// RTS line state. ActiveHigh (default) means the line is
	// driven high while sending data. ActiveLow means the line
	// is driven low while sending data.
	Rts RtsState
	// Delay between RTS state change and data transmission
	DelayBefore time.Duration
	// Delay between the end of data transmission and RTS state
	// change.
	DelayAfter time.Duration
}

// Convert to the RS485 configuration struct used by the serial package.
func (rc RS485Configuration) export() serial.RS485Config {
	var src serial.RS485Config
	if rc.Mode == 0 {
		// RS485 not enabled
		return src
	}
	src.Enabled = true
	src.DelayRtsBeforeSend = rc.DelayBefore
	src.DelayRtsAfterSend = rc.DelayAfter
	if rc.Mode == FullDuplex {
		src.RxDuringTx = true
	}
	switch rc.Rts {
	case ActiveHigh:
		src.RtsHighDuringSend = true
	case ActiveLow:
		src.RtsHighAfterSend = true
	}
